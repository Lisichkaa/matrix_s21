...
START_TEST(test_s21_sub_matrix_1) {
  ...
  for (int i = 0; i < A.rows; i++) {
    for (int j = 0; j < A.columns; j++) {
      A.matrix[i][j] = 1;
      B.matrix[i][j] = 1;
      reference.matrix[i][j] = 0;
    }
  }

  response = s21_sub_matrix(&A, &B, &result);
...
START_TEST(test_s21_sub_matrix_3) {
  ...
  response = s21_sub_matrix(&A, &B, NULL);
...
Suite *s21_s21_sub_matrix(void) {
  ...
  /* Core test case */
  tc_core = tcase_create("test_s21_sub_matrix");
  ...
  tcase_add_test(tc_core, test_s21_sub_matrix_9);
